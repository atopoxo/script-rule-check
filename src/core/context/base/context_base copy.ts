...
import {ContextOption, ContextItem} from '../../ai_model/base/ai_types'
...
type Parser = (content: string, types: IdentifierType[], startPos: number, dependencyGraph?: Map<string, Set<string>>) => ContextTreeNode;

export class ContextBase {
    protected languageParsers: {[key: string]: Parser} = {
        lua: (content, types, startPos, dependencyGraph) => this.parseLua(content, types, startPos, dependencyGraph),
        py: (content, types, startPos, dependencyGraph) => this.parsePython(content, types, startPos, dependencyGraph),
        js: (content, types, startPos, dependencyGraph) => this.parseJsTs(content, types, startPos, dependencyGraph),
        ts: (content, types, startPos, dependencyGraph) => this.parseJsTs(content, types, startPos, dependencyGraph),
        c: (content, types, startPos, dependencyGraph) => this.parseCpp(content, types, startPos, dependencyGraph),
        cc: (content, types, startPos, dependencyGraph) => this.parseCpp(content, types, startPos, dependencyGraph),
        h: (content, types, startPos, dependencyGraph) => this.parseCpp(content, types, startPos, dependencyGraph),
        hpp: (content, types, startPos, dependencyGraph) => this.parseCpp(content, types, startPos, dependencyGraph),
        cpp: (content, types, startPos, dependencyGraph) => this.parseCpp(content, types, startPos, dependencyGraph)
    };
    ...
    protected getRelatedContext(startPos: number, text: string, filePath: string): Map<string, ContextItem[]> {
        ...
        const {tree, content} = this.getIdentifiers(0, filePath, undefined, dependencyGraph);
        if (!tree) {
            return result;
        }
        const items: ContextItem[] = [];
        this.getIdentifiersByRange(items, tree, startPos, startPos + text.length);
        ...
        this.findRelatedContext(queue, filePath, content,tree, result, visited, 0, maxDepth, dependencyGraph);
        return result;
    }

    protected getIdentifiers(startPos: number, filePath: string, content?: string, dependencyGraph?: Map<string, Set<string>>): {tree: ContextTreeNode | undefined, content: string} {
        try {
            const ext = path.extname(filePath).toLowerCase().substring(1);
            const parser = this.languageParsers[ext];
            if (!parser) {
                return {tree: undefined, content: ''};
            }
            if (!content) {
            ...
            const result = parser(content, types, startPos, dependencyGraph);
            
            return {tree: result, content: content};
        } catch (error) {
    ...
    private getIdentifiersByRange(result: ContextItem[], current: ContextTreeNode, startPos: number, endPos: number) {
        if (!current.value) {
            return;
        }
    ...
    protected findRelatedContext(queue: Deque<ContextItem>,
        ...
        const trieTree = new Trie<ContextTreeNode>();
        this.tree2Trie(trieTree, tree);
        const identifierMap = new Map<string, ContextTreeNode>();
        this.traverseTree(identifierMap, tree);
        ...
        if (!missQueue.isEmpty()) {
            ...
            for (const include of includes) {
                ...
                if (includePath && fs.existsSync(includePath) && depth < maxDepth) {
                    const includeIdentifiers = this.getIdentifiers(0, includePath, undefined, dependencyGraph);
                    if (!includeIdentifiers.tree) {
                        continue;
                    }
                    this.findRelatedContext(queue, includePath, includeIdentifiers.content, includeIdentifiers.tree, result, visited, depth + 1, maxDepth, dependencyGraph);
                }
            }
        }
    }

    private traverseTree(result: Map<string, ContextTreeNode>, current: ContextTreeNode) {
    ...
    private tree2Trie(result: Trie<ContextTreeNode>, current: ContextTreeNode) {
        const value = current.value;
        if (value) {
    ...
    protected parseLua(content: string, types: IdentifierType[], startPos: number, dependencyGraph?: Map<string, Set<string>>): ContextTreeNode {
        ...
        this.traverseLua(ast, (root.value as ContextItem).name, types, root, content, startPos, dependencyGraph);
        return root;
    }
    ...
    protected parsePython(content: string, types: IdentifierType[], startPos: number, dependencyGraph?: Map<string, Set<string>>): ContextTreeNode {
        const Parser = require('tree-sitter');
        const parser = new Parser();
    ...
    protected parseJsTs(content: string, types: IdentifierType[], startPos: number, dependencyGraph?: Map<string, Set<string>>): ContextTreeNode {
        const Parser = require('tree-sitter');
        const parser = new Parser();
    ...
    protected parseCpp(content: string, types: IdentifierType[], startPos: number, dependencyGraph?: Map<string, Set<string>>): ContextTreeNode {
        const Parser = require('tree-sitter');
        const parser = new Parser();
    ...
    protected traverseLua(node: any, parentName: string, types: IdentifierType[], parent: ContextTreeNode | undefined, content: string, startPos: number, dependencyGraph?: Map<string, Set<string>>) {
        ...
        switch (node.type) {
            ...
            case 'AssignmentStatement':
                current = this.luaAssignmentStatement(node, parentName, types, content, startPos, dependencyGraph);
                break;
            ...
            case 'ForGenericStatement':
                ...
                break;
            case 'CallExpression':
                current = this.luaCallExpression(node, parentName, types, content, startPos, dependencyGraph);
                break;
            ...
            if (node.hasOwnProperty(key) && typeof node[key] === 'object') {
                if (Array.isArray(node[key])) {
                    for (const child of node[key]) {
                        if (child) {
                            this.traverseLua(child, currentName, types, current, content, startPos, dependencyGraph);
                        }
                    }
                } else {
                    if (node[key]) {
                        this.traverseLua(node[key], currentName, types, current, content, startPos, dependencyGraph);
                    }
                }
            }
        }
    ...
    private luaAssignmentStatement(node: any, parentName: string, types: IdentifierType[], content: string, startPos: number, dependencyGraph?: Map<string, Set<string>>): ContextTreeNode | undefined {
        let current: ContextTreeNode | undefined = undefined;
        for (const type of types) {
            switch (type) {
                case 'variable':
                    ...
                    if (dependencyGraph) {
                        ...
                        for (const varNode of node.variables) {
                            if (varNode.type === 'Identifier') {
                                const fullName = `${current.value?.name}>${varNode.name}`;
                                leftVars.push(fullName);
                            }
                        }
                        const rightDeps = new Set<string>();
                        for (const expr of node.init) {
                            this.collectDependenciesFromExpression(expr, rightDeps, parentName, content, startPos);
                        }
    ...
    private luaCallExpression(node: any, parentName: string, types: IdentifierType[], content: string, startPos: number, dependencyGraph?: Map<string, Set<string>>): ContextTreeNode | undefined {
        let current: ContextTreeNode | undefined = undefined;
        for (const type of types) {
            switch (type) {
                case 'function':
                ...
                case 'variable':
                    if (dependencyGraph) {
                        if (node.base.type === 'Identifier') {
                            ...
                            const name = `${parentName}>${start}~${end}>${node.base.name}`;
                            if (!dependencyGraph.has(name)) {
                                dependencyGraph.set(name, new Set());
                            }
                            const funcDeps = dependencyGraph.get(name)!;
                            const deps = new Set<string>();
                            for (const arg of node.arguments) {
                                this.collectDependenciesFromExpression(arg, deps, name, content, startPos);
                            }
    ...
    private collectDependenciesFromExpression(node: any, dependencies: Set<string>, parentName: string, content: string, startPos: number) {
        let name = '';
        if (!node) {
            return name;
        }
        switch (node.type) {
            case 'Identifier':
                const start: number = node.range[0] + startPos;
                const end: number = node.range[1] + startPos;
                name = `${parentName}>${start}~${end}>${node.name}`;
                dependencies.add(name);
                break;
            case 'CallExpression':
                if (node.base) {
                    this.collectDependenciesFromExpression(node.base, dependencies, parentName, content, startPos);
                }
                if (node.arguments) {
                    for (const arg of node.arguments) {
                        this.collectDependenciesFromExpression(arg, dependencies, parentName, content, startPos);
                    }
                }
                break;            
            case 'MemberExpression':
                if (node.base) {
                    name = this.collectDependenciesFromExpression(node.base, dependencies, parentName, content, startPos);
                }
                if (node.identifier) {
                    this.collectDependenciesFromExpression(node.identifier, dependencies, name, content, startPos);
                }
                break;
            ...
            default:
                for (const key in node) {
                    if (node.hasOwnProperty(key) && typeof node[key] === 'object') {
                        if (Array.isArray(node[key])) {
                            for (const child of node[key]) {
                                this.collectDependenciesFromExpression(child, dependencies, parentName, content, startPos);
                            }
                        } else {
                            this.collectDependenciesFromExpression(node[key], dependencies, parentName, content, startPos);
                        }
                    }
                }
                break;
    ...
    protected createContextItemForLua(node: any, type: string, parentName: string, needRange: boolean, name: string | undefined, content: string, startPos: number, replaceContent?: string): ContextItem {
        const start: number = node.range[0] + startPos;
        const end: number = node.range[1] + startPos;
        if (needRange) {
        ...
        const result = {
            type: type,
            name: name,
            content: replaceContent? replaceContent : content.substring(start, end),
            range: {
                start: start,
                end: end,
